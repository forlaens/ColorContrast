...
<main>
	<noscript>
		For this tool to work, your browser must have JavaScript enabled.
	</noscript>

	<form id="step-1" class="step" action="" method="POST" enctype="multipart/form-data" onsubmit="loadImagePreview(); return false;">
		<h1>Image contrast checker</h1>
	...
	<div hidden id="step-2" class="step">
		<h1>Highlight contrast issues</h1>

		<input type="button" class="cta" onclick="resetFileInput(); showStep(1);" value="New image">

		<input hidden type="button" id="reset-image" class="cta gray" onclick="updatePreviewCanvas();" value="Reset image">

		<div hidden role="status" class="loading" aria-atomic="true"></div>

		<region id="preview_area" aria-label="Contrast checker">
			<div role="toolbar" aria-label="Settings">
				<div class="group vcenter">
					<input type="color" name="color" aria-labelledby="testcolor-label">
					<button id="colorpicker" class="swatch" aria-label="Pick a color from the image" aria-pressed="false" onclick="toggleColorPicker(this);">
					...
					<div id="testcolor-label" class="label-text" aria-hidden="true">
						Color to test
					</div>
				</div>
				
				<hr class="vcenter" aria-orientation="vertical">
				...
				<div class="group vcenter">
					<label>
						...
						<span id="contrast-label" class="label-text" aria-hidden="true">
							Conformance level
						</span>
					</label>
				</div>
				...
				<div class="group vcenter">
					<button class="cta" onclick="initRenderContrast();">
